...
const app = createApp({
  ...
  methods: {
    ...
    selectImportFormat(f) { this.importFormat = f; this.importLoaded = false; },
    loadImportPreview() {
      if (this.importFormat === 'JSON' && !this.importLang) { this.toast('请先选择语种'); return; }
    ...
    doImport() {
      ...
      this.toast(this.importFormat === 'JSON' ? `已导入「${this.langLabel(this.importLang)}」语种文案（演示）` : `已导入 ${this.importFormat} 全部语种文案（演示）`);
    },
