...
const app = createApp({
  ...
  methods: {
    ...
    // 某个页面在其所在项目内的链式展示（页面 / 子页面，不含项目名）
    pageChainDisplay(project, pageId) {
      if (!project) return '';
      if (('projcopies_' + project.id) === pageId) return '（项目级文案）';
      const path = this.findPagePath(pageId, project.pages) || [];
      return path.map(p => p.name).join(' / ');
    },
    // 定位某条文案在项目中的第一个所属页面（用于构造 demo 行）
    ...
    buildDedupGroups(project) {
      ...
      if (project.name === 'VeSync App') {
        return [
          // 组1：本项目 2 条（不同页面）+ 其它项目 1 条，英文均为 "Delete"
          g([
          ...
          // 组2：本项目 1 条 + 其它项目 1 条，英文均为 "Settings"
          g([
            this.dedupRow(vesync, 'Settings', true, 'Settings'),
            this.dedupRow(site, 'Contact_Us', false, 'Settings'),
          ]),
          // 组3：本项目 2 条（分属不同页面 home / device_schedule）+ 其它项目 2 条（分属不同项目），英文均为 "Add"
          g([
      ...
      // 其它项目：构造一组（本项目 1 条 + 其它项目 1 条，英文一致）作为通用演示
